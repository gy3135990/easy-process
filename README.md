...
Easy Process 流程设计器，基于vue3 + vite4 + element-plus实现，具备低代码、快速应用及扩展的特点。
...
| 设计器组件脱离element-plus | 进行中 | 设计器组件的样式及事件实现完全脱离第三方组件，提升可移植性 |
|                            |        |                                                            |
...
│        └─ base						// 通用组件
│          └─ BaseNode.vue				// 通用节点组件（该组件只是节点的外壳，具体内容由各节点组件部署实现）
│          └─ BaseDrawer.vue			// 通用节点属性配置组件（该组件只是节点属性配置的外壳，具体内容由各节点组件部署实现）
│          └─ AddNode.vue				// 添加节点组件
│        └─ router						// 路由节点
